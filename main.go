...
import (
	...
	volumeSN "github.com/Ae-Mc/VolumeSerialNumber"
)

func help() {
	fmt.Println("Wrong arguments count")
...
func serialNumberToString(volume_sn uint64) string {
	...
	var volume_sn_bytes [8]byte
	copy(volume_sn_bytes[:], buf.Bytes())
	return fmt.Sprintf(
...
func printVolumeSerialNumber(drive string) {
	volume_sn, err := volumeSN.GetVolumeSerialNumber(drive)
	if err != nil {
...
func setVolumeSerialNumber(drive string, volume_sn uint64) {
	err := volumeSN.SetVolumeSerialNumber(drive, volume_sn)
	if err != nil {
		fmt.Println("Error:", err.Error())
		os.Exit(1)
	}
...
func userInputToSerialNumber(user_input string) (volume_sn uint64, err error) {
	volume_sn_str := strings.ToUpper(strings.Replace(user_input, "-", "", -1))
...
func main() {
	...
	is_supported := drive_type == DRIVE_TYPE_FIXED ||
		drive_type == DRIVE_TYPE_REMOVABLE
	if !is_supported {
